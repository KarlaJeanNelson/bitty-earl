...
module.exports = class myUrl {
	static exists(req, res, next) {
		...
		axios(testUrl)
			...
			.catch(e =>
				res
					.status(400)
					.json({ error: true, message: `Address ${longurl} not found` })
			);
	}
		...
		Url.create(newUrl, (err, doc) => {
			err
				...
				: res
						...
						.json({
							error: false,
							message: `New url added with id ${doc._id}`
						});
		});
	}
	...
	static delete(req, res) {
		Url.findOneAndDelete({ _id: req.params.id }, (err, doc) => {
			err
				...
				: res
						.status(200)
						.json({ error: false, message: `Record ${doc._id} deleted` });
		});
	}
	...
	static increment(req, res, next) {
		...
		if (longUrl.error || longUrl.status >= 400) {
			res
				.status(longUrl.status)
				.json({ error: true, message: longUrl.message });
		}
		...
		if (req.user) {
			Url.findOneAndUpdate(
				{ longurl: longUrl, _id: req.user._id },
				{ $inc: { hitcount: 1 } }
			)
				.then(doc => {
					!doc
						? res.status(200).json({ error: true, message: `No record found` })
						: res
								.status(200)
								.json({
									error: false,
									message: `Another hit for record ${doc._id}!`
								});
				})
				.catch(err =>
					res.status(400).json({ error: true, message: err.message })
			...
			Url.updateMany({ longurl: longUrl }, { $inc: { hitcount: 1 } })
				.then(docs =>
					!docs
						...
						: res
								...
								.json({
									error: false,
									message: `${docs.length} records updated!`
								})
				)
